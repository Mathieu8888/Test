...
MAJOR_STOCKS = {
    ...
    "🇺🇸 Telecom & Services": ["T", "VZ", "TMUS", "CMCSA", "CHTR", "DIS", "NFLX", "PARA", "WBD", "FOXA"],
    
    "🇪🇺 Europe (ADR)": ["ASML", "SAP", "NVO", "AZN", "HSBC", "BP", "SHEL", "BHP", "RIO", "BTI"],
    
    "🌏 International (ADR)": ["TSM", "BABA", "PDD", "JD", "NIO", "BIDU", "TME", "BILI", "LI", "XPEV"]
}
...
def get_stock_data(ticker):
    ...
    try:
        ...
        # Validation : filtrer les données aberrantes
        # Market cap doit être entre 1M$ et 10T$
        if market_cap and (market_cap < 1_000_000 or market_cap > 10_000_000_000_000):
            return None
        
        # Prix doit être > 0 et < 100000$
...
with tab_top100:
    ...
    st.info("📊 **Actions US & ADR** - Les actions non-US sont filtrées pour éviter les erreurs de conversion de devises. Focus sur NYSE, NASDAQ et ADR liquides.")
    ...
    with st.spinner("📊 Chargement des données..."):
        ...
        for idx, ticker in enumerate(ALL_STOCKS[:150]):  # Charger plus pour avoir 100 valides
            status_text.text(f"Chargement: {ticker} ({idx+1}/{min(150, len(ALL_STOCKS))})")
            data = get_stock_data(ticker)
            if data and data['market_cap'] > 0:
                stock_data.append(data)
            progress_bar.progress((idx + 1) / min(150, len(ALL_STOCKS)))
            time.sleep(0.05)  # Réduire le délai
        
        progress_bar.empty()
        status_text.empty()
        
        st.success(f"✅ {len(stock_data)} actions chargées avec succès")
